...
export async function collectorPresetEditor(detail, _data) {
    ...
    try {
        ...
        detail.replaceChildren(head(), el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '불러오지 못했습니다 — ' + e.message })));
        return;
    }
    ...
    const body = el('div', {});
    body.append(el('p', { class: 'admin-hint' }, ...uiText('라이블리가 기본으로 아는 도구(슬랙·노션 등) 외에, 사내 API·공개 피드·웹훅을 코드 없이 붙일 수 있습니다. 여기서 만든 방식은 [맥락 관리 ▸ 수집]에서 수집기를 만들 때 고를 수 있게 됩니다.')));
    ...
    bcard.append(el('div', { class: 'ctx-cov-row' }, ...builtin.map((p) => el('span', { class: 'ctx-tag', text: p.label }))));
    body.append(bcard);
    if (!custom.length && !creating) {
        body.append(el('div', { class: 'card ctx-empty' }, el('p', { class: 'ctx-empty-t', text: '직접 만든 수집 방식이 없습니다' }), el('p', { class: 'admin-hint', text: '사내 시스템의 API 나 팀 블로그 피드처럼 기본 제공에 없는 소스를 붙이려면 여기서 방식을 정의하세요.' })));
    }
    ...
    if (creating)
        body.append(presetForm(null, builtin, reload));
    else {
...
function head() {
    return sectionHead('수집 방식', '기본 제공에 없는 소스를 코드 없이 붙입니다. 여기서 정의한 방식으로 [맥락 관리 ▸ 수집]에서 수집기를 만듭니다.');
}
function presetSummary(p, used, reload) {
    const card = el('div', { class: 'card ctx-row' });
    card.append(el('div', { class: 'ctx-row-head' }, el('span', { class: 'ctx-row-title', text: p.label }), el('span', { class: 'ctx-tag', text: DRIVER_LABEL[p.driver] || p.driver }), el('span', { class: 'ctx-state' }, el('span', { class: 'ctxp-dot ' + (p.enabled ? 'ctxp-dot-ok' : 'ctxp-dot-off'), 'aria-hidden': 'true' }), el('span', { text: p.enabled ? '사용' : '미사용' })), used ? el('span', { class: 'ctx-tag', text: `수집기 ${used}개가 사용 중` }) : null, p.parser_script ? el('span', { class: 'ctx-tag', title: '응답을 변환하는 스크립트가 붙어 있습니다', text: '커스텀 파서' }) : null));
    if (p.description)
        card.append(el('div', { class: 'ctx-row-meta', text: p.description }));
    ...
    del.addEventListener('click', async () => {
        if (used) {
            toast(`이 방식으로 만든 수집기가 ${used}개 있습니다 — 먼저 정리하세요`, true);
            return;
        }
        ...
        if (!ok)
            return;
        try {
            await api(`/api/ui/org/collector-presets/${p.id}/remove`, { method: 'POST', body: '{}' });
            toast('삭제했습니다');
            reload();
        }
        catch (e) {
...
function presetForm(p, builtin, reload) {
    const isNew = !p;
    ...
    const cfg = (p?.driverConfig ?? {});
    const map = (cfg.map ?? {});
    const driverSel = el('select', { style: S, ...(isNew ? {} : { disabled: true }) });
    for (const [k, l] of Object.entries(DRIVER_LABEL))
        driverSel.append(el('option', { value: k, text: l }));
    driverSel.value = p?.driver ?? 'http';
    const driverWhat = el('p', { class: 'admin-hint ctx-field-hint', text: DRIVER_WHAT[driverSel.value] });
    const keyIn = el('input', { type: 'text', style: S, value: p?.key ?? '', placeholder: 'intra-notice' });
    const labelIn = el('input', { type: 'text', style: S, value: p?.label ?? '', placeholder: '사내 공지 API' });
    const descIn = el('input', { type: 'text', style: S, value: p?.description ?? '', placeholder: '목록에 보일 한 줄 설명' });
    const enabledChk = el('input', { type: 'checkbox' });
    enabledChk.checked = p ? p.enabled !== false : true;
    // ── clone ──
    const baseSel = el('select', { style: S });
    baseSel.append(el('option', { value: '', text: '— 복제할 기본 제공 선택 —' }));
    for (const b of builtin)
        baseSel.append(el('option', { value: b.key, text: b.label }));
    if (p?.basePreset)
        baseSel.value = p.basePreset;
    ...
    const urlIn = el('input', { type: 'text', style: S, value: cfg.url ?? '',
        placeholder: 'https://intra.example.com/api/posts?page={page}' });
    const authKindSel = el('select', { style: S });
    for (const [v, t] of [['none', '없음'], ['bearer', 'Bearer 토큰'], ['header', '헤더에 토큰'], ['basic', '아이디·비밀번호'], ['query', '주소에 토큰']]) {
        authKindSel.append(el('option', { value: v, text: t }));
    }
    authKindSel.value = cfg.auth?.kind ?? 'none';
    const authNameIn = el('input', { type: 'text', style: S, value: cfg.auth?.name ?? '', placeholder: 'X-Api-Key (헤더 방식)' });
    const pagKindSel = el('select', { style: S });
    for (const [v, t] of [['none', '한 번만 호출'], ['page', '쪽 번호 (?page=1,2,3…)'], ['offset', '건너뛴 개수'], ['cursor', '다음 커서를 응답에서'], ['link', '다음 주소를 응답에서']]) {
    ...
    const itemsIn = el('input', { type: 'text', style: S, value: cfg.itemsPath ?? '', placeholder: '$.data.items — 비우면 응답 자체가 목록' });
    const privChk = el('input', { type: 'checkbox' });
    privChk.checked = cfg.allowPrivateNetwork === true;
    const incChk = el('input', { type: 'checkbox' });
    ...
    for (const [k, label, hint] of MAP_FIELDS) {
        const i = el('input', { type: 'text', style: S, value: map[k] ?? '', placeholder: `$.${k}` });
        mapIn[k] = i;
        mapRows.append(el('div', { class: 'ctx-map-row' }, el('div', { class: 'ctx-map-k' }, el('span', { text: label }), hint ? el('span', { class: 'ctx-map-h', text: hint }) : null), i));
    }
    const httpBox = el('div', {}, F('호출 주소', '{page}·{offset}·{cursor} 와 설정값 {config.키} 를 넣을 수 있습니다.', urlIn), F('인증 방식', '토큰 값 자체는 여기 적지 않습니다 — 수집기마다 따로 넣습니다(계정이 다를 수 있으니까).', authKindSel), F('헤더·파라미터 이름', 'header·query 방식일 때만. 비우면 Authorization / access_token.', authNameIn), F('여러 쪽 가져오기', '', pagKindSel), F('다음 쪽 위치', 'cursor·link 방식일 때, 응답에서 다음 값을 찾을 경로.', pagNextIn), F('목록 위치', '응답에서 항목 배열이 있는 곳. 비우면 응답 전체를 목록으로 봅니다.', itemsIn), el('label', { class: 'admin-check' }, incChk, ' 바뀐 것만 가져오기 — 주소에 {cursor} 를 넣으면 마지막으로 가져온 시각이 들어갑니다(예: ?since={cursor}).'), el('p', { class: 'admin-hint ctx-field-hint' }, ...uiText('끄면 매번 전체를 다시 읽습니다. 같은 항목은 덮어써지므로 중복은 안 생기지만, 항목이 많으면 낭비입니다. 켜려면 위 짝짓기에서 수정 시각(또는 작성 시각)도 채워야 합니다 — 그 값으로 어디까지 읽었는지 기억합니다.')), el('label', { class: 'admin-check' }, privChk, ' 사내망 주소 허용 — ⚠ 기본은 사설·내부 주소를 막습니다(실수로 내부 시스템을 긁지 않게). 사내 API 를 붙일 때만 켜세요.'));
    // ── RSS ──
    const rssUrlIn = el('input', { type: 'text', style: S, value: cfg.url ?? '', placeholder: 'https://blog.example.com/feed.xml' });
    const rssPrivChk = el('input', { type: 'checkbox' });
    rssPrivChk.checked = cfg.allowPrivateNetwork === true;
    const rssBox = el('div', {}, F('피드 주소', 'RSS 2.0 · Atom 둘 다 됩니다. 제목·본문·작성자·시각은 자동으로 알아냅니다.', rssUrlIn), el('label', { class: 'admin-check' }, rssPrivChk, ' 사내망 주소 허용 — 사내 피드일 때만 켜세요.'));
    // ── webhook ──
    const sigKindSel = el('select', { style: S });
    ...
    const sigHeaderIn = el('input', { type: 'text', style: S, value: cfg.signature?.header ?? '', placeholder: 'X-Hub-Signature-256' });
    const sigPrefixIn = el('input', { type: 'text', style: S, value: cfg.signature?.prefix ?? '', placeholder: 'sha256=' });
    const whItemsIn = el('input', { type: 'text', style: S, value: cfg.itemsPath ?? '', placeholder: '$.events — 비우면 본문 전체가 항목 1건' });
    const webhookBox = el('div', {}, el('p', { class: 'admin-hint ctx-field-hint' }, ...uiText('저장하고 수집기를 만들면 받을 주소가 생깁니다 — [맥락 관리 ▸ 수집]의 그 수집기 설정에서 확인해 보내는 쪽에 등록하세요.')), F('서명 검증', '보내는 쪽과 공유 비밀을 나눠 갖고 서명을 확인합니다. 끄면 주소만 알면 누구나 우리 저장소에 넣을 수 있습니다.', sigKindSel), F('서명 헤더 이름', '보내는 쪽 문서에 적혀 있습니다.', sigHeaderIn), F('서명 접두어', 'sha256= 처럼 값 앞에 붙는 것이 있으면 적으세요.', sigPrefixIn), F('목록 위치', '한 번에 여러 건이 오면 그 배열의 경로. 비우면 본문 전체가 한 건입니다.', whItemsIn));
    // 매핑 표는 http·webhook 공용이다 — **노드는 하나**고 자리만 옮긴다(같은 표를 두 벌 만들면 입력이 갈린다).
    const mapHint = el('p', { class: 'admin-hint ctx-field-hint' });
    const mapHost = el('div', { class: 'ctx-field' }, el('div', { class: 'field-label', text: '가져온 값을 우리 항목에 짝짓기' }), mapHint, mapRows);
    // ── 커스텀 파서 ──
    ...
    const parserBox = el('details', { class: 'ctx-guide', ...(p?.parserScript ? { open: '' } : {}) }, el('summary', { text: '커스텀 파서 (선택) — 짝짓기만으로 안 될 때' }));
    parserBox.append(el('p', { class: 'admin-hint' }, ...uiText('응답을 우리 항목으로 바꾸는 코드를 직접 씁니다. 한 응답에 종류가 섞여 있어 갈라야 하거나, 본문이 조각나 있어 합쳐야 할 때 씁니다. parse(input) 함수를 정의하면 그 반환값이 항목 목록이 됩니다.')), el('p', { class: 'ctx-danger-note' }, ...uiText('⚠ 이 코드는 서버에서 실행됩니다. 파일·다른 프로그램 접근은 막혀 있고 10초 안에 끝나야 하지만, 네트워크는 막지 못합니다 — 받은 데이터를 밖으로 보낼 수 있다는 뜻입니다. 신뢰하는 코드만 넣으세요.')), parserIn);
    const boxes = { http: httpBox, rss: rssBox, webhook: webhookBox, clone: cloneBox };
    const syncDriver = () => {
        driverWhat.textContent = DRIVER_WHAT[driverSel.value];
        for (const [k, box] of Object.entries(boxes))
            box.hidden = k !== driverSel.value;
    ...
    const saveBtn = el('button', { class: 'btn btn-primary', text: isNew ? '수집 방식 만들기' : '저장' });
    ...
    card.append(el('div', { class: 'ctx-row-head' }, el('span', { class: 'ctx-row-title', text: isNew ? '새 수집 방식' : `설정 — ${p.label}` })), F('어떤 방식인가', null, el('div', {}, driverSel, driverWhat, 
    // ⚠ raw append 에 null 을 넘기면 DOM 이 문자열 "null" 노드를 만든다(el() 과 달리 안 걸러 준다).
    //  그래서 조건부 요소는 el() 안에 넣는다 — el() 이 null 자식을 무시한다.
    isNew ? null : el('p', { class: 'admin-hint ctx-field-hint', text: '방식은 만든 뒤 바꾸지 않습니다 — 설정 항목이 통째로 달라집니다.' }))), F('식별자', '영문·숫자 슬러그. 기본 제공 이름(slack·notion 등)은 쓸 수 없습니다.', keyIn), F('이름', '수집기를 만들 때 목록에 보일 이름입니다.', labelIn), F('설명', '', descIn), cloneBox, httpBox, rssBox, webhookBox, mapHost, parserBox, el('label', { class: 'admin-check' }, enabledChk, ' 이 방식 사용 — 끄면 새 수집기를 만들 때 목록에 안 보입니다'), el('div', { class: 'ctx-actions' }, saveBtn, cancelBtn));
    syncDriver();
    return card;
}
