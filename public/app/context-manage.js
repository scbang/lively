...
export async function renderFindings(host) {
    ...
    try {
        ...
        if (findingFilter.kind)
            qs.set('kind', findingFilter.kind);
        if (findingFilter.severity)
            qs.set('severity', findingFilter.severity);
    ...
    const body = el('div', {});
    body.append(el('p', { class: 'admin-hint' }, ...uiText('관리기가 찾아낸 것들입니다. 확인해서 고치거나, 문제가 아니면 반려하세요 — 반려한 것은 다음 검사에서 다시 올라오지 않습니다.')));
    ...
    for (const s of ['high', 'warn']) {
        filters.append(chip(SEV_LABEL[s], findingFilter.severity === s, () => { findingFilter.severity = s; reload(); }));
    }
    body.append(filters);
    if (!findings.length) {
        body.append(el('div', { class: 'card ctx-empty' }, el('p', { class: 'ctx-empty-t', text: '처리할 발견이 없습니다' }), el('p', { class: 'admin-hint', text: findingFilter.kind || findingFilter.severity ? '이 조건에 해당하는 것이 없습니다. 필터를 풀어 보세요.' : '관리기가 아직 아무것도 찾지 못했거나, 전부 처리했습니다.' })));
        host.replaceChildren(body);
        return;
    }
    // '조치안 제시'(propose) 관리기가 낸 **적용 가능한** 발견 — 한 번에 적용할 수 있다(#1419 T9).
    //  report 와의 실질 차이가 여기다: report 는 목록만, propose 는 이 묶음 바가 뜬다.
    const proposed = findings.filter((f) => f.actionable && f.action_level === 'propose');
    if (proposed.length) {
        ...
        btn.addEventListener('click', async () => {
            ...
            try {
                const r = await api('/api/ui/org/manager-findings/apply', {
                    method: 'POST', body: JSON.stringify({ ids: proposed.map((f) => f.id) })
                });
        ...
        bar.append(el('span', { class: 'ctx-bulk-t', text: '적용 대기' }), el('span', { class: 'ctx-bulk-d', text: '관리기가 조치안까지 만들어 뒀습니다. 확인하고 한 번에 적용하세요.' }), btn);
        body.append(bar);
    }
    const list = el('div', { class: 'ctx-findings' });
    for (const f of findings)
        list.append(findingRow(f, reload));
    body.append(list);
    host.replaceChildren(body);
}
function findingRow(f, reload) {
    const card = el('div', { class: 'card ctx-finding ctx-sev-' + f.severity });
    const head = el('div', { class: 'ctx-finding-head' }, el('span', { class: 'ctx-sev', text: SEV_LABEL[f.severity] || f.severity }), el('span', { class: 'ctx-tag', text: KIND_LABEL[f.kind] || f.kind }), f.seen_count > 1 ? el('span', { class: 'ctx-tag', title: '이 문제가 반복해서 발견된 횟수', text: `${f.seen_count}회 발견` }) : null, 
    // '적용 대기' — 조치안이 준비돼 있고 관리기가 propose 모드일 때만(#1419 T9). report 모드에선 안 뜬다.
    (f.actionable && f.action_level === 'propose')
        ? el('span', { class: 'ctx-tag ctx-tag-ready', title: '관리기가 조치안까지 만들어 뒀습니다', text: '적용 대기' }) : null, el('span', { class: 'ctx-finding-when', text: relTime(f.last_seen_at) }));
    card.append(head);
    ...
    if (f.evidence) {
        const det = el('details', { class: 'ctx-evidence' }, el('summary', { text: '근거 보기' }), el('p', { class: 'ctx-evidence-b', text: f.evidence }));
        card.append(det);
    }
    const acts = el('div', { class: 'ctx-row-acts' });
    const pa = f.proposed_action;
    ...
    const done = el('button', { class: 'btn btn-ghost btn-sm', text: '직접 고침' });
        ...
        try {
            await api(`/api/ui/org/manager-findings/${f.id}/resolve`, { method: 'POST', body: JSON.stringify({ state: 'resolved' }) });
            toast('처리했습니다');
            reload();
        }
        catch (e) {
    ...
    reject.addEventListener('click', async () => {
        try {
            await api(`/api/ui/org/manager-findings/${f.id}/resolve`, { method: 'POST', body: JSON.stringify({ state: 'rejected' }) });
            toast('반려했습니다 — 다시 올라오지 않습니다');
            reload();
        }
        catch (e) {
            toast(e.message, true);
...
export async function renderManagers(host) {
    ...
    const body = el('div', {});
    body.append(el('p', { class: 'admin-hint' }, ...uiText('쌓인 지식이 낡거나 어긋나지 않게 자동으로 살핍니다. 찾아낸 것은 [발견]에 쌓이고, 사람이 확인해서 고치거나 반려합니다.')));
    ...
    for (const k of ov.by_kind || []) {
        grid.append(el('div', { class: 'ctx-kind' }, el('div', { class: 'ctx-kind-h' }, el('span', { class: 'ctxp-dot ' + (k.enabled ? 'ctxp-dot-ok' : 'ctxp-dot-off'), 'aria-hidden': 'true' }), el('span', { class: 'ctx-kind-t', text: k.label })), el('div', { class: 'ctx-kind-m', text: k.enabled ? `확인 필요 ${fmtNum(k.open)}건` : '꺼짐' })));
    }
    kinds.append(grid);
    body.append(kinds);
    if (!list.length && !creating) {
        body.append(el('div', { class: 'card ctx-empty' }, el('p', { class: 'ctx-empty-t', text: '아직 관리기가 없습니다' }), el('p', { class: 'admin-hint', text: '분류 어긋남·낡은 지식은 계산만으로 찾아낼 수 있어 비용이 들지 않습니다 — 그 둘부터 켜 보세요.' })));
    }
    for (const m of list)
        body.append(editingKey === m.key ? managerEditor(m, reload) : managerSummary(m, reload));
    if (creating)
        body.append(managerEditor(null, reload));
    else {
...
function managerSummary(m, reload) {
    const card = el('div', { class: 'card ctx-row' });
    card.append(el('div', { class: 'ctx-row-head' }, el('span', { class: 'ctx-row-title', text: m.label || m.key }), el('span', { class: 'ctx-tag', text: KIND_LABEL[m.kind] || m.kind }), el('span', { class: 'ctx-state' }, el('span', { class: 'ctxp-dot ' + (m.enabled ? 'ctxp-dot-ok' : 'ctxp-dot-off'), 'aria-hidden': 'true' }), el('span', { text: m.enabled ? '켜짐' : '꺼짐' })), el('span', { class: 'ctx-tag', text: ACTION_LABEL[m.action_level] || m.action_level }), m.open_findings ? el('span', { class: 'ctx-tag ctx-tag-warn', text: `확인 필요 ${fmtNum(m.open_findings)}` }) : null));
    ...
    toggle.addEventListener('click', async () => {
        try {
            await api('/api/ui/org/managers', { method: 'POST', body: JSON.stringify({ id: m.id, enabled: !m.enabled }) });
            toast('저장했습니다');
            reload();
        }
        catch (e) {
    ...
    del.addEventListener('click', async () => {
        ...
        if (!ok)
            return;
        try {
            await api('/api/ui/org/managers/remove', { method: 'POST', body: JSON.stringify({ id: m.id }) });
            toast('삭제했습니다');
            reload();
        }
        catch (e) {
...
function managerEditor(m, reload) {
    const isNew = !m;
    ...
    const kindSel = el('select', { style: S, ...(isNew ? {} : { disabled: true }) });
    for (const [k, l] of Object.entries(KIND_LABEL))
        kindSel.append(el('option', { value: k, text: l }));
    if (m)
    ...
    const keyIn = el('input', { type: 'text', style: S, value: m?.key ?? '', placeholder: 'drift-product', ...(isNew ? {} : { disabled: true }) });
    const labelIn = el('input', { type: 'text', style: S, value: m?.label ?? '', placeholder: '제품 도메인 어긋남 검사' });
    const enabledChk = el('input', { type: 'checkbox' });
    enabledChk.checked = m ? !!m.enabled : true;
    const actionSel = el('select', { style: S });
    ...
    const spacesIn = el('input', { type: 'text', style: S, value: (m?.match_spaces || []).join(', '), placeholder: '비우면 전체' });
    const catsIn = el('input', { type: 'text', style: S, value: (m?.match_categories || []).join(', '), placeholder: '비우면 전체' });
    // 민감도 — kind 마다 뜻이 다르므로 라벨·힌트를 바꿔 단다(같은 칸에 다른 의미가 들어가면 잘못 넣는다).
    const thIn = el('input', { type: 'number', style: S, step: '0.05', value: m?.threshold != null ? String(m.threshold) : '' });
    const staleIn = el('input', { type: 'number', style: S, value: m?.stale_days != null ? String(m.stale_days) : '' });
    ...
    const syncSensitivity = () => {
        ...
        const lbl = thField.querySelector('.field-label');
        const hint = thField.querySelector('.ctx-field-hint');
        if (k === 'mismatch') {
            if (lbl)
                lbl.textContent = '어긋남 판정 기준';
            if (hint)
                hint.textContent = '다른 분류에 이만큼 더 가까우면 어긋났다고 봅니다. 낮출수록 많이 잡히고 오탐도 늘어납니다. 비우면 0.1.';
            thIn.placeholder = '0.1';
        }
        else if (k === 'contradiction') {
            if (lbl)
                lbl.textContent = '후보로 볼 유사도';
            if (hint)
                hint.textContent = '이만큼 닮은 지식 쌍만 AI 에게 보냅니다. 낮출수록 많이 검사하고 비용도 늡니다. 비우면 0.85.';
            thIn.placeholder = '0.85';
        }
    };
    const critIn = el('textarea', { style: S + ';min-height:90px;resize:vertical',
        placeholder: '예) 고객 대응 문서는 표현이 달라도 결론이 같으면 모순이 아니다.' });
    critIn.value = m?.criteria_md ?? '';
    const batchIn = el('input', { type: 'number', style: S, min: '1', max: '200', value: String(m?.batch_size ?? 20) });
    const reqIn = el('input', { type: 'text', style: S, value: m?.requester ?? '', placeholder: '판단이 필요한 종류에만 쓰입니다' });
    const saveBtn = el('button', { class: 'btn btn-primary', text: isNew ? '관리기 만들기' : '저장' });
    ...
    card.append(el('div', { class: 'ctx-row-head' }, el('span', { class: 'ctx-row-title', text: isNew ? '새 관리기' : `설정 — ${m.label || m.key}` })), F('무엇을 검사할까', null, el('div', {}, kindSel, kindWhat)), isNew ? null : el('p', { class: 'admin-hint ctx-field-hint', text: '검사 종류는 만든 뒤 바꿀 수 없습니다 — 판정 기준이 통째로 달라 기존 발견이 섞입니다.' }), F('식별자', '영문·숫자 슬러그.', keyIn), F('이름', '목록에 보일 이름입니다.', labelIn), F('찾은 뒤 어떻게 할까', '자동으로 고치는 것은 되돌릴 수 있는 조치에만 적용됩니다. 처음에는 “찾기만 함”으로 두고 결과를 보세요.', actionSel), F('검사할 갈래', '이 space 만 검사합니다. 비우면 전체.', spacesIn), F('검사할 분류축', '이 분류축의 지식만 검사합니다. 비우면 전체.', catsIn), thField, staleField, F('판단 기준', '이 조직에서 무엇을 문제로 볼지 적으세요. 판단이 필요한 종류(모순·코드 비교)의 AI 지시문에 들어갑니다.', critIn), F('한 번에 검사할 수', '', batchIn), F('의뢰자', '모순·코드 비교는 AI 가 판정합니다 — 이 사람의 계정으로 실행·과금됩니다.', reqIn), el('label', { class: 'admin-check' }, enabledChk, ' 이 관리기 사용'), el('div', { class: 'ctx-actions' }, saveBtn, cancelBtn));
    syncSensitivity();
    return card;
}
