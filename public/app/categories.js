...
function mismatchLevel(c) {
    if (!c.mismatch_measurable || c.mismatch_count == null)
        return 'unmeasured';
...
async function renderCategoriesInner(view, withHead) {
    ...
    for (const s of SPACE_SUBS)
        bySpace[s.key] = cats.filter((c) => c.space === s.key);
    const head = withHead
        ? pageHead('분류체계', '지식과 프로젝트를 어떤 갈래로 나눌지 정합니다. 정의가 오래되면 여기서 먼저 드러납니다.', [], '분류체계')
        : el('p', { class: 'admin-hint' }, '지식과 프로젝트를 어떤 갈래로 나눌지 정합니다. 분류기는 여기 적힌 정의(범위·규칙)를 기준으로 판단하므로, 정의가 비면 분류 기준도 없습니다.');
    ...
    if (noDef.length || mismatched.length) {
        const bits = [];
        if (noDef.length)
            bits.push(el('span', { class: 'pill pill-warn', text: `정의 없음 ${noDef.length}` }));
        if (mismatched.length)
            bits.push(el('span', { class: 'pill pill-warn', text: `분류 어긋남 ${mismatched.length}` }));
        summary.append(el('div', { class: 'wikicat-summary-row' }, ...bits, el('span', { class: 'wikicat-summary-txt' }, ...uiText('정의(범위·규칙)가 비어 있거나, 담긴 지식이 다른 분류의 정의에 더 가깝습니다. 행을 눌러 무엇인지 보고 — 정의를 넓히거나, 그 지식을 옮기세요.'))));
    }
    else if (cats.length) {
        summary.append(el('div', { class: 'wikicat-summary-row' }, el('span', { class: 'pill pill-ok', text: '정의와 내용 일치' }), el('span', { class: 'wikicat-summary-txt', text: '모든 분류에 정의가 있고, 정의에서 크게 벗어난 지식도 없습니다.' })));
    }
    const list = el('div', { class: 'wikicat' });
    for (const s of SPACE_SUBS) {
        const items = bySpace[s.key] || [];
        const isProduct = s.key === 'product';
        const groupHead = el('div', { class: 'wikicat-grouphead' }, el('span', { class: 'wikicat-grouptitle', text: s.label }), isProduct ? el('span', { class: 'dm-tag', text: '도메인' }) : null, el('span', { class: 'wikicat-groupcount', text: String(items.length) }));
        if (canEdit) {
        ...
        if (!items.length) {
            ...
            for (const c of items)
                rows.append(categoryRow(c, s.key, { canEdit, teams, repos, reload }));
        }
        list.append(el('div', { class: 'wikicat-group' }, groupHead, rows));
    }
    view.replaceChildren(...[
        head,
        canEdit ? null : el('p', { class: 'admin-hint' }, el('span', { class: 'pill', text: '읽기 전용' }), ' 편집은 context 권한이 필요합니다.'),
        summary,
        list,
    ].filter(Boolean));
}
// 한 행 — 이름·키·오너 팀·정의 한 줄 + 표류 배지 + (제품) 연결 레포. 액션은 hover 시 진해진다(wikicat-row-acts).
function categoryRow(c, space, ctx) {
    ...
    const shouldLine = should
        ...
        : el('span', { class: 'wikicat-should wikicat-should-empty' }, el('span', { class: 'wikicat-should-label', text: '정의·범위·규칙' }), canEdit ? uiText('미설정 — 오른쪽 [수정]에서 입력할 수 있어요') : '미설정');
    ...
    if (should) {
        ...
        if (lv === 'unmeasured') {
            ...
            mismatchEl = el('span', { class: 'wikicat-drift', title: '정의와 내용의 대조는 의미 검색(임베딩)이 켜져 있어야 합니다. 켜져 있다면 아직 계산 대기 중입니다.' }, el('span', { class: 'wikicat-drift-date', text: '대조 전' }));
        }
        else if (lv === 'none') {
            mismatchEl = el('span', { class: 'wikicat-drift', title: '이 분류의 지식이 모두 다른 어떤 분류의 정의보다 이 정의에 가깝습니다.' }, el('span', { class: 'wikicat-drift-date', text: '정의와 일치' }));
        }
        else {
            // 경보 색은 기존 .pill-warn 을 그대로 얹는다(새 색 리터럴을 만들지 않는다 — DS 컬러 예산).
            mismatchEl = el('span', { class: 'wikicat-drift' + (lv === 'warn' ? ' pill pill-warn' : ''),
                title: '이 분류의 정의보다 다른 분류의 정의에 더 가까운 지식입니다. 눌러서 무엇인지 보고 — 정의를 넓히거나 그 지식을 옮기세요.' }, el('span', { class: 'wikicat-drift-date', text: '다른 분류에 더 가까움' }), el('span', { class: 'wikicat-drift-n', text: fmtNum(n) + '건' }));
        }
    }
    // 오너 팀 — 카테고리 소유(표면화·주입의 '우리 팀' 기준). 오너십=우선순위이지 접근제한이 아니다.
    let ownerEl = null;
    if (canEdit) {
        const ownerSel = el('select', { class: 'wikicat-owner-sel', 'aria-label': '오너 팀' }, el('option', { value: '', text: '— 오너 없음 —' }), ...teams.map((t) => el('option', { value: String(t.id), text: t.name || t.key })));
    ...
    const linked = Array.isArray(c.repos) ? c.repos : [];
    const repoEl = linked.length
        ? el('span', { class: 'wikicat-repos', title: '이 분류가 사는 코드 레포(명시 설정)' }, el('span', { class: 'wikicat-repos-label', text: '레포' }), el('span', { class: 'wikicat-repos-v', text: linked.join(' · ') }))
        : null;
    const main = el('div', { class: 'wikicat-row-main' }, el('span', { class: 'wikicat-name', text: c.name || c.key }), el('span', { class: 'wikicat-key mono', text: c.key }), c.cross_cutting ? el('span', { class: 'dm-tag', text: '횡단' }) : null, mismatchEl, ownerEl, repoEl, shouldLine);
    const acts = canEdit ? el('div', { class: 'wikicat-row-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: '수정',
        onclick: () => openCategoryForm(space, c, reload, { repos }) }), el('button', { class: 'btn btn-ghost btn-sm btn-text-danger', text: '삭제',
    ...
    if (should && Number(c.mismatch_count || 0) > 0) {
        ...
        toggle.addEventListener('click', async () => {
            ...
            try {
                ...
                loaded = true;
                if (!items.length) {
                    detail.replaceChildren(el('div', { class: 'wikicat-empty', text: '어긋난 지식이 없습니다.' }));
                    return;
                }
                const list = el('div', { class: 'wikicat-mismatch-list' });
                for (const m of items) {
                    list.append(el('div', { class: 'wikicat-mismatch-row' }, el('a', { class: 'wikicat-mismatch-t', href: '#/k/' + encodeURIComponent(m.name), text: m.title || m.name }), m.nearest_name ? el('span', { class: 'wikicat-mismatch-to', text: '→ ' + m.nearest_name }) : null, el('span', { class: 'wikicat-mismatch-d', title: '이 분류의 정의보다 저 분류의 정의에 이만큼 더 가깝습니다', text: String(m.margin) })));
                }
                detail.replaceChildren(el('p', { class: 'admin-hint', style: 'margin:0 0 8px' }, ...uiText('이 분류의 정의보다 다른 분류의 정의에 더 가까운 지식입니다. 정의가 이들을 품어야 하면 [수정]에서 정의를 넓히고, 아니면 화살표가 가리키는 분류로 옮기세요.')), list);
            }
            catch (e) {
...
async function deleteCategory(c, reload) {
