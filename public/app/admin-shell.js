...
function membersSection(detail, data) {
    ...
    detail.replaceChildren(sectionHead('구성원 · 팀', '이 조직에 누가 있는지 보고 고치며, 팀으로 묶습니다.', data.meaning['member']), segTabs('members', [
...
function memberAccessSection(detail, data) {
    detail.replaceChildren(sectionHead('구성원 권한 관리', '구성원이 무엇으로 라이블리에 접속하는지, 그 사람의 AI가 어느 계정으로 실행되는지 관리합니다.'), segTabs('member-access', [
