...
const ORG_AUDIT_STATE = { scope: 'admin', entity: '', actor_kind: '', channel: '', op: '', period: 'all', from: '', to: '', page: 1 };
const ORG_AUDIT_PERIODS = [['all', '전체 기간'], ['1d', '최근 24시간'], ['7d', '최근 7일'], ['30d', '최근 30일'], ['90d', '최근 90일']];
const ORG_AUDIT_PERIOD_DAYS = { all: 0, '1d': 1, '7d': 7, '30d': 30, '90d': 90 };
...
async function orgAuditPanel(detail) {
    ...
    const controls = el('div', { class: 'aud-filters' }, audPeriodField(ORG_AUDIT_PERIODS, ORG_AUDIT_STATE, 'period', () => { ORG_AUDIT_STATE.page = 1; reload(); }), audField('범위', scopeSel), mkSel('종류', 'entity', entityOpts, '모든 종류'), mkSel('행위자', 'actor_kind', kindOpts, '사람·AI 전체'), mkSel('경로', 'channel', chanOpts, '모든 경로'), mkSel('작업', 'op', opOpts, '모든 작업'), audPageSizeField('org', () => { ORG_AUDIT_STATE.page = 1; reload(); }), el('div', { class: 'aud-right' }, anyFilter ? el('button', { class: 'btn btn-ghost btn-sm', text: '필터 해제',
        onclick: () => { ORG_AUDIT_STATE.entity = ''; ORG_AUDIT_STATE.actor_kind = ''; ORG_AUDIT_STATE.channel = ''; ORG_AUDIT_STATE.op = ''; ORG_AUDIT_STATE.page = 1; reload(); } }) : null, audExportBtn(() => audExportCsv('org', new URLSearchParams(filterQs()))), el('button', { class: 'btn btn-ghost btn-sm', text: '새로고침', onclick: reload })));
    // ── 행 리스트(펼치면 필드별 이전→이후) ──
