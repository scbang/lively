...
async function openCronForm(job, actions, reload, tz) {
    ...
    let distillers = null; // #1289 증류기 피커 — 한 번만 받아 재사용(액션 전환 시 재요청 안 함)
    let classifiers = null; // #1419 T4 분류기 피커 — 같은 캐시 규칙
    let managers = null; // #1419 T5 관리기 피커 — 같은 캐시 규칙
    async function renderParams() {
        ...
        for (const p of (a.params || [])) {
            ...
            if (p.kind === 'session') {
                ...
                if (distillers == null) {
                    try {
                        const r = await api('/api/ui/org/distillers');
                        distillers = (r && r.distillers) || [];
                    }
                    catch {
                        distillers = [];
                    }
                ...
                for (const d of (distillers || []))
                    inp.append(el('option', { value: d.key, text: (d.label || d.key) + (d.enabled ? '' : ' (꺼짐)') }));
                ...
                for (const c of (classifiers || []))
                    inp.append(el('option', { value: c.key, text: (c.label || c.key) + (c.enabled ? '' : ' (꺼짐)') }));
                ...
                if (managers == null) {
                    try {
                        const r = await api('/api/ui/org/managers');
                ...
                for (const g of (managers || []))
                    inp.append(el('option', { value: g.key, text: (g.label || g.key) + ' · ' + g.kind + (g.enabled ? '' : ' (꺼짐)') }));
    ...
    const form = el('div', { class: 'proj-settings' }, psBlock('잡 id', isNew ? '소문자 슬러그(a-z0-9_-). 잡의 고유 키.' : 'id 는 변경 불가.', idInp), psBlock('이름', '관리 목록에 보일 이름.', labelInp), psBlock('액션', '게이트웨이가 실행할 작업(등록된 액션 레지스트리). 액션마다 필요한 인자가 아래에 자동으로 뜹니다.', actionSel), paramsWrap, psBlock('주기 (초)', '이 간격마다 실행(최소 60). cron식이 있으면 그게 우선.', intervalInp), psBlock('cron식 (선택)', '벽시계 스케줄 — 시각은 ' + tz + ' 기준입니다. 예: 0 9 * * 1-5 = 평일 ' + tz + ' 09:00. 비우면 주기초.', cronInp), psBlock('한 번만 실행', '체크 시 주기·cron 무시 → 1회 실행 후 자동으로 꺼짐(반복 안 함). 부트스트랩 등 일회성 잡용.', el('label', { class: 'inline' }, onceChk, el('span', { text: ' run once (1회 실행 후 비활성)' }))), psBlock('켬', '', el('label', { class: 'inline' }, enabledChk, el('span', { text: ' 활성화' }))), el('div', { class: 'ps-rules-actions' }, saveBtn));
...
function openManagedSessionForm(m, reload) {
    ...
    const form = el('div', { class: 'proj-settings' }, psBlock('세션 id', isNew ? '소문자 슬러그(a-z0-9_-). 고유 키.' : 'id 는 변경 불가.', idInp), psBlock('이름', '관리 목록·세션 탭에 보일 이름.', labelInp), psBlock('라이블리 계정/프로필', '이 세션을 띄울 클로드 로그인(프로필=구성원). 목록에서 고르거나 입력. 각 프로필은 provision + 웹터미널 /login 후 사용.', account.el), psBlock('격리 워크스페이스(하위경로)', '공유폴더 아래 이 세션 전용 작업폴더. 비우면 managed/<id>.', wsInp), psBlock('하네스', '', harnessSel), psBlock('모델 (claude)', '이 세션의 claude 모델. 판단 무거운 작업(부트스트랩·분류)은 opus 권장. 비우면 기본.', modelSel), psBlock('effort (claude)', '추론 강도(low~max). 무거운 판단은 high+ 권장. 비우면 기본.', effortSel), psBlock('자동 승인', '도구 실행을 묻지 않고 진행(무인 작업에 필요).', el('label', { class: 'inline' }, autoChk, el('span', { text: ' --dangerously-skip-permissions' }))), psBlock('항상 켬(keep-alive)', '죽으면 재생성.', el('label', { class: 'inline' }, enabledChk, el('span', { text: ' enabled' }))), el('div', { class: 'ps-rules-actions' }, saveBtn));
...
export { cronPanel, managedSessionsPanel, };
