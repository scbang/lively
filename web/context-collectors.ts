...
export async function renderCollectors(host: HTMLElement): Promise<void> {
  ...
  if (!collectors.length && !creatingPreset && !choosingPreset) {
    body.append(el('div', { class: 'card ctx-empty' },
      ...
      el('p', { class: 'admin-hint', text: '외부 도구를 연결하면 그 내용이 자료로 쌓이고, 다음 단계(증류)가 그중 남길 가치가 있는 것을 지식으로 만듭니다.' })));
  }
  ...
  if (choosingPreset) body.append(presetChooser(presets, reload));
...
function collectorSummary(c: any, reload: () => void) {
  ...
  card.append(el('div', { class: 'ctx-row-head' },
    ...
    el('span', { class: 'ctx-tag', text: c.preset_label }),
  ...
  if (c.last_run) meta.push(`최근 ${relTime(c.last_run.started_at)} · ${c.last_run.status === 'ok' ? '성공' : c.last_run.status === 'running' ? '진행 중' : '실패'}`);
  else meta.push('아직 실행된 적 없음');
  ...
  toggle.addEventListener('click', async () => {
    ...
    try {
      ...
      toast(c.enabled ? '껐습니다' : '켰습니다 — 자동 수집이 등록됐습니다'); reload();
    } catch (e) { toast((e as Error).message, true); (toggle as HTMLButtonElement).disabled = false; }
  });
  ...
  del.addEventListener('click', async () => {
    ...
    if (!ok) return;
    try { await api(`/api/ui/org/collectors/${c.id}/remove`, { method: 'POST', body: '{}' }); toast('삭제했습니다'); reload(); }
...
function presetChooser(presets: any[], reload: () => void) {
  ...
  const grid = (list: any[]) => {
    ...
    for (const p of list) {
      ...
      b.append(
        el('span', { class: 'ctx-preset-t', text: p.label }),
        el('span', { class: 'ctx-preset-d', text: p.description || driverHint(p.driver) }));
  ...
  card.append(el('p', { class: 'admin-hint', style: 'margin-top:12px' },
    ...uiText('찾는 도구가 없나요? [설정 ▸ 수집 방식]에서 사내 API·RSS·웹훅을 코드 없이 직접 정의할 수 있습니다.')));
...
function collectorEditor(c: any | null, presets: any[], reload: () => void, newPreset?: any) {
  ...
  const keyIn = el('input', { type: 'text', style: S, value: c?.key ?? '',
    placeholder: '비우면 자동 생성', ...(isNew ? {} : { disabled: true }) }) as HTMLInputElement;
  ...
  // 주기
  const intervalSel = el('select', { style: S }) as HTMLSelectElement;
  ...
  // 산출 정책(T3)
  const outSel = el('select', { style: S }) as HTMLSelectElement;
  ...
  const noteIn = el('input', { type: 'text', style: S, value: c?.note ?? '', placeholder: '선택 사항' }) as HTMLInputElement;
  ...
  if (preset.guide?.steps?.length) {
    ...
    guideEl = el('details', { class: 'ctx-guide', ...(hasToken ? {} : { open: '' }) },
      el('summary', { text: `${preset.label} 연결 방법` }));
    if (preset.guide.intro) guideEl.append(el('p', { class: 'admin-hint', text: preset.guide.intro }));
  ...
  const saveBtn = el('button', { class: 'btn btn-primary', text: isNew ? '수집기 만들기' : '저장' }) as HTMLButtonElement;
  saveBtn.addEventListener('click', async () => {
    ...
    const missing = (preset.fields || []).filter((f: any) =>
      f.required && !(f.secret ? (secrets[f.key] || c?.secretsSet?.[f.key]) : config[f.key]));
    if (missing.length) { toast(`${missing.map((f: any) => f.label || f.key).join(', ')} 을(를) 입력하세요`, true); return; }
  ...
  card.append(
    el('div', { class: 'ctx-row-head' },
      el('span', { class: 'ctx-row-title', text: isNew ? `새 수집기 — ${preset.label}` : `설정 — ${c.label || c.key}` })),
    guideEl,
    F('이름', '목록에 보일 이름입니다.', labelIn),
    F('식별자', isNew ? '영문·숫자 슬러그. 비우면 자동으로 만듭니다.' : '만든 뒤에는 바꾸지 않습니다.', keyIn),
    ...fieldEls,
    F('수집 주기', '자주 가져올수록 최신이지만 외부 서비스 호출도 늘어납니다.', intervalSel),
    F('수집 결과', '가져온 내용을 자료로 쌓을지, 바로 지식으로 만들지 정합니다.', outSel),
    catField,
    F('메모', null, noteIn),
    el('label', { class: 'admin-check' }, enabledChk, ' 이 수집기 사용 — 켜면 위 주기로 자동 수집합니다'),
    actions);
  return card;
}
...
async function openRuns(c: any) {
  ...
  try {
    ...
    if (!runs.length) { box.replaceChildren(el('p', { class: 'admin-hint', text: '실행 이력이 없습니다.' })); return; }
