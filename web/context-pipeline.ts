...
function jobHealth(job: any, whatRuns: string): Health | null {
  if (!job) return { level: 'off', line: `${whatRuns} 자동 실행이 등록돼 있지 않습니다 — 설정해도 아무것도 돌지 않습니다.` };
  if (!job.any_enabled) return { level: 'off', line: `${whatRuns} 자동 실행이 꺼져 있습니다 — 켜야 돕니다.` };
  return null;
}

function collectHealth(s: any): Health {
  if (!s.configured) return { level: 'off', line: '수집기가 없습니다. 슬랙·노션 같은 곳을 연결해 자료를 모으세요.' };
  if (!s.enabled) return { level: 'off', line: `수집기 ${s.configured}개가 모두 꺼져 있습니다.` };
...
function distillHealth(s: any): Health {
  ...
  if (j) return { ...j, line: j.line + (s.backlog ? ` 밀린 자료 ${fmtNum(s.backlog)}건이 그대로 쌓입니다.` : '') };
  if (!s.configured) return { level: 'note', line: '증류기가 없어 전 자료 공통 기본 증류로 돕니다. 채널별로 기준을 나누려면 만드세요.' };
  if (!s.enabled) return { level: 'off', line: `증류기 ${s.configured}개가 모두 꺼져 있습니다.` };
  if (s.backlog > 1000) return { level: 'warn', line: `지식이 안 된 자료가 ${fmtNum(s.backlog)}건 밀려 있습니다.` };
  if (s.backlog) return { level: 'note', line: `지식이 안 된 자료 ${fmtNum(s.backlog)}건이 대기 중입니다.` };
...
function manageHealth(s: any): Health {
  if (!s.configured) return { level: 'note', line: '관리기가 없습니다. 분류 어긋남·낡은 지식을 자동으로 찾게 하세요.' };
  if (!s.enabled) return { level: 'off', line: `관리기 ${s.configured}개가 모두 꺼져 있습니다.` };
  const j = jobHealth(s.job, '관리');
  if (j) return j;
  if (s.open.high) return { level: 'warn', line: `확인이 필요한 발견 ${fmtNum(s.open.high)}건(중요)이 있습니다.` };
  if (s.open.total) return { level: 'note', line: `확인이 필요한 발견 ${fmtNum(s.open.total)}건이 있습니다.` };
  return { level: 'ok', line: '처리할 발견이 없습니다.' };
}

/** 상태 점 — 채운 색 필을 쓰지 않는다(DS: 채운 컬러 상태 필 금지 → 점+라벨). */
function dot(level: Health['level']) {
...
const LEVEL_TEXT: Record<Health['level'], string> = { ok: '정상', note: '참고', warn: '주의', off: '멈춤' };

/** 단계 카드 하나. 숫자 두 개(산출·잔량)와 상태 한 줄, 그리고 그 단계로 가는 입구. */
function stageCard(o: {
  step: number; title: string; what: string; health: Health;
  metrics: Array<{ label: string; value: string; hint?: string }>;
  goto: string; gotoLabel: string;
}) {
  const card = el('a', {
  ...
  card.append(
    el('div', { class: 'ctxp-stage-head' },
      el('span', { class: 'ctxp-step', text: String(o.step) }),
      el('span', { class: 'ctxp-stage-title', text: o.title }),
      el('span', { class: 'ctxp-state' }, dot(o.health.level), el('span', { text: LEVEL_TEXT[o.health.level] }))),
    el('p', { class: 'ctxp-what', text: o.what }),
    el('p', { class: 'ctxp-line', text: o.health.line }),
    el('div', { class: 'ctxp-metrics' },
      ...o.metrics.map((m) => el('div', { class: 'ctxp-metric', ...(m.hint ? { title: m.hint } : {}) },
        el('span', { class: 'ctxp-metric-v', text: m.value }),
        el('span', { class: 'ctxp-metric-l', text: m.label })))),
    el('span', { class: 'ctxp-goto', text: o.gotoLabel + ' →' }));
  return card;
}

/** 단계 사이의 흐름 표시 — 무엇이 넘어가는지(자료 → 지식 → 분류된 지식). */
function flow(label: string) {
  return el('div', { class: 'ctxp-flow', 'aria-hidden': 'true' },
    el('span', { class: 'ctxp-flow-line' }),
    el('span', { class: 'ctxp-flow-label', text: label }),
    el('span', { class: 'ctxp-flow-line' }));
}
...
export async function renderPipeline(host: HTMLElement): Promise<void> {
  host.replaceChildren(el('div', { class: 'ctxp-loading' },
    el('p', { class: 'admin-hint', text: '파이프라인 현황을 불러오는 중…' })));

  let d: any;
  try { d = await api('/api/ui/org/pipeline'); }
  catch (e) {
    host.replaceChildren(el('div', { class: 'card' },
      el('p', { class: 'admin-hint', text: '현황을 불러오지 못했습니다 — ' + (e as Error).message })));
    return;
  }
  const s = d.stages;
  ...
  const healths: Array<[string, Health]> = [
  ...
  if (blocked.length) {
    summary.append(
      el('span', { class: 'ctxp-summary-badge', text: `${blocked.length}단계 확인 필요` }),
      el('span', { class: 'ctxp-summary-txt', text: blocked.map(([n]) => n).join(' · ') + ' 단계가 멈췄거나 밀려 있습니다. 아래에서 해당 단계를 열어 보세요.' }));
  } else {
    summary.append(
      el('span', { class: 'ctxp-summary-badge ctxp-summary-ok', text: '정상 흐름' }),
      el('span', { class: 'ctxp-summary-txt', text: '네 단계가 모두 돌고 있습니다.' }));
  }
  wrap.append(summary);
  ...
  if (g.knowledge_pending || g.classification_proposed) {
    const gates = el('div', { class: 'ctxp-gates' },
      el('span', { class: 'ctxp-gates-title', text: '사람 확인 대기' }));
    if (g.knowledge_pending) {
      gates.append(el('a', { class: 'ctxp-gate', href: '#/knowledge/review' },
        el('span', { class: 'ctxp-gate-n', text: fmtNum(g.knowledge_pending) }),
        el('span', { class: 'ctxp-gate-l', text: '지식 검토 대기' })));
    }
    if (g.classification_proposed) {
      gates.append(el('a', { class: 'ctxp-gate', href: '#/knowledge/classifications' },
        el('span', { class: 'ctxp-gate-n', text: fmtNum(g.classification_proposed) }),
        el('span', { class: 'ctxp-gate-l', text: '분류 검토 대기' })));
    }
    wrap.append(gates);
  }

  // 자동 실행 상태 — 네 잡을 한 줄로. '무엇이 언제 도는가'가 파이프라인 신뢰의 근거다.
  const jobs = el('div', { class: 'ctxp-jobs' },
    el('span', { class: 'ctxp-jobs-title', text: '자동 실행' }));
  for (const [name, st] of [['수집', s.collect], ['증류', s.distill], ['분류', s.classify], ['관리', s.manage]] as Array<[string, any]>) {
    const j = st.job;
    jobs.append(el('span', { class: 'ctxp-job' },
      dot(!j || !j.any_enabled ? 'off' : 'ok'),
      el('span', { class: 'ctxp-job-n', text: name }),
      el('span', { class: 'ctxp-job-v', text: !j ? '미등록'
        : !j.any_enabled ? '꺼짐'
