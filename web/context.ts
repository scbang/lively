...
const inner: Record<string, string> = { classify: 'classifiers', manage: 'findings' };
...
export async function renderContext(view: HTMLElement, sub?: string | null): Promise<void> {
  ...
  if (sel === 'overview') { await renderPipeline(host); return; }
  if (sel === 'collect') { await renderCollectors(host); return; }
  if (sel === 'distill') {
    // 증류기 화면은 관리탭 패널을 그대로 재사용한다 — 같은 데이터에 두 화면을 만들지 않는다.
    //  그 패널은 admin 데이터(meaning 등)를 인자로 받으므로 여기서 채워 준다(없으면 빈 객체로 동작).
    let data: any = {};
    try { data = await loadAdmin(); } catch { /* 권한 없거나 실패 — 패널은 자기 API 로 그린다 */ }
    host.replaceChildren();
    await distillersPanel(host, data);
    return;
  }
  if (sel === 'classify') { await renderClassifyStage(host); return; }
  await renderManageStage(host);
}

/** 분류 단계 — '분류기'(규칙)와 '분류축'(갈래 정의) 2단 탭. */
async function renderClassifyStage(host: HTMLElement) {
  const box = el('div', {});
  const body = el('div', {});
...
async function drawClassifyInner(body: HTMLElement) {
  body.replaceChildren(skeleton('불러오는 중'));
  if (inner.classify === 'categories') await renderCategoryList(body);
  else await renderClassifiers(body);
}

/** 관리 단계 — '발견'(일감)이 먼저, '관리기'(설정)가 뒤. */
async function renderManageStage(host: HTMLElement) {
  const box = el('div', {});
  const body = el('div', {});
...
async function drawManageInner(body: HTMLElement) {
  body.replaceChildren(skeleton('불러오는 중'));
  if (inner.manage === 'managers') await renderManagers(body);
  else await renderFindings(body);
}

/** 2단 탭(세그먼티드) — 관리탭 segTabs 와 같은 시각 언어. 라우터를 안 타므로 인메모리 상태로. */
function segmented(scope: string, items: Array<{ key: string; label: string }>, body: HTMLElement) {
  ...
  for (const it of items) {
    ...
    b.addEventListener('click', () => {
      if (inner[scope] === it.key) return;
      inner[scope] = it.key;
      for (const other of bar.querySelectorAll('.ctx-seg-item')) {
        const on = (other as HTMLElement).textContent === it.label;
